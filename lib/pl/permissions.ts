 ...
 */
export async function getCaptainSide(teamAId: string, teamBId: string): Promise<{ side: PlSide; username: string } | null> {
  ...
  if (teamIds.includes(teamAId)) return { side: "A", username: user.name }
  if (teamIds.includes(teamBId)) return { side: "B", username: user.name }
  return null
}
