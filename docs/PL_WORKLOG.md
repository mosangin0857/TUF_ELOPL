...
- [x] Supabase SQL Editor에서 `docs/sql/006_pl_entry_rules.sql` 실행 (지정 세트 컬럼 · 제출 기록 테이블)
...
- 엔트리: 팀장 · 부팀장 둘 다 제출. members.role에는 추가하지 않고 pl_team_members 역할로 판단
- A팀(왼쪽) = 홈, B팀(오른쪽) = 원정. 세트 형식 · 맵은 관리자가 경기 등록 때 지정(엔트리 공개 때 맵도 보이도록)
- 지정 세트: 정규 라운드 '홈 지정', 플레이오프 · 결승 '홈 지정' · '어웨이 지정'. 지정 팀이 개인전(정해진 맵) 또는 팀플 2:2 · 3:3 · 4:4(맵풀에서 맵 선택)을 고름 → 즉시 상대에게 형식 · 맵 공개, 한 번 고르면 관리자만 되돌림
- 엔트리 마감 = 공개 2시간 전 (공개 19시 → 17시 마감). 마감 뒤에는 제출 · 수정 · 형식 선택 불가
...
### 2026-09-24 — 엔트리 규칙: 홈/원정 · 지정 세트 · 마감 + 엔트리 화면 개선
**한 일**
- 엔트리 제출 화면 목업 제작 (claude.ai 아티팩트) → 추가 기능 4개 확정
- DB: `docs/sql/006_pl_entry_rules.sql` (pl_sets.pick_by · solo_map · picked_at, pl_entry_logs, 실행 완료)
- PL 관리 › 경기 등록에 **세트 구성** 추가: 세트별 지정(없음 · 홈 지정 · 어웨이 지정) · 형식 · 맵. 지정 세트의 맵은 '개인전이면 쓰는 맵'. 이미 고른 지정 세트는 '되돌리기'
- 엔트리 제출 화면 개편: 홈/원정 표시, 마감까지 남은 시간(공개 2시간 전), 우리 팀 입력 수, 상대 제출 여부(세트 수만), 지정 세트 형식 고르기(개인전/팀플 + 맵풀 맵, 확정 확인), 상대가 고를 세트는 대기 안내, 선수별 출전 세트 수, 제출 기록, 지난 경기 엔트리 불러오기, 하단 진행률 · 제출 바
- 서버: 마감 · 지정 세트 규칙 검사(`submitEntryAction`, `pickSetFormatAction`, `resetPickAction`), 제출 기록 저장
- 일정: 지정 세트 표시(홈 지정 · 선택 전 / 형식 · 맵), 엔트리 버튼은 마감 전에만
- `npm run typecheck` · `npm run build` 통과

- 로컬 확인 후 main 반영 (시즌 TFPL_S4 생성 · 경기 등록 · 엔트리 제출 테스트 흔적 있음)

**다음에 할 일**
- 실제 시즌 데이터 입력 (테스트로 넣은 경기 · 기록 정리)
- 시즌 삭제 기능이 없음 → 필요하면 추가
